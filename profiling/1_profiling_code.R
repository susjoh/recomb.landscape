...
profvis({
  ...
  r1   <- sample(map.dist/100, n.loci)  #0.02
  r2   <- sample(map.dist/100, n.loci)  #0.02
  rhet <- (r1 + r2)/2
  ...
  mafs.found <- sample(maf.info, n.loci) #0.04
  ...
  founder.haplos <- lapply(1:n.found.hap, function (x) (runif(n.loci) < mafs.found) + 0L) #0.25
  ...
  ref.0$Bred <- 0
  ref.0$Bred[sort(c(which(ref.0$SEX == 1 & ref.0$PHENO >= m.thresh), #0.03
